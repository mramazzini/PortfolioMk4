...
const SideNav1 = () => {
  ...
  const handlePhoneClick = () => {
    ...
    <div className = {`side-nav ${inViewport ? 'in-viewport' : ''}`}>
        ...
        <Link key={icons[2].id} className = {`side-nav-icon   ${icons[2].show ? 'show' : 'hide'}`} to="#" target="_blank" onClick={(e) => {
        ...
        <Link key={icons[4].id} className = {`side-nav-icon   ${icons[4].show ? 'show' : 'hide'}`} target="_blank" to="https://github.com/mramazzini" ><GithubOutlined /></Link>
        <div  key={icons[5].id}className = {`side-nav-divider ${icons[5].show ? 'show' : 'hide'}`}></div>
        
        <Link key={icons[6].id} className = {`side-nav-icon   ${icons[6].show ? 'show' : 'hide'}`} target="_blank" to="https://www.linkedin.com/in/matteo-ramazzini/" ><LinkedinOutlined /></Link>
